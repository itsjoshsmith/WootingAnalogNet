...
using static WootingAnalogNet.WootingAnalog;

namespace WootingAnalogNet.Test
{
    ...
    static void Main(string[] args)
    {
      ...
      try
      {
        ...
        if (DeviceCount > 0 && Initialised)
        {
          ...
          Keyboard.WaitKeyPress(EScanCode1.Enter);
          ...
          while (Keyboard.IsKeyFullyDown(EScanCode1.Esc) == false)
          {
            ...
            if (Count >= 0)
            {
              ...
              {
                if(KeyCodesBuffer[i] != (ushort)EScanCode1.Esc)
                {
                  switch (Keyboard.KeyCodeMode)
                  {
                    case EKeyCodeMode.ScanCode1:
                      Console.WriteLine($"Key: 0x{KeyCodesBuffer[i]:X2} ({(EScanCode1)KeyCodesBuffer[i]}) | Value: {AnalogReadingsBuffer[i]} | Percent: {Keyboard.ValueToPercent(AnalogReadingsBuffer[i]).ToString("0")}%");
                      break;
                    case EKeyCodeMode.HID:
                      Console.WriteLine($"Key: 0x{KeyCodesBuffer[i]:X2} ({(EHIDKey)KeyCodesBuffer[i]}) | Value: {AnalogReadingsBuffer[i]} | Percent: {Keyboard.ValueToPercent(AnalogReadingsBuffer[i]).ToString("0")}%");
                      break;
                    case EKeyCodeMode.VirtualKey:
                      Console.WriteLine($"Key: 0x{KeyCodesBuffer[i]:X2} ({(EVirtualKey)KeyCodesBuffer[i]}) | Value: {AnalogReadingsBuffer[i]} | Percent: {Keyboard.ValueToPercent(AnalogReadingsBuffer[i]).ToString("0")}%");
                      break;
                  }
                }
              }
          ...
          if (CheckLastStatus(LastResult, "ClearDeviceCallback") == false) return;
